function [] = rundriver(inseed)
f = inseed; %Use arbitrary seed values 143 - 153 that stand out when post-processing. 
rng(f);
successTrials = [1 1;
                 1 5;
                 1 10];
...
b = successTrials(j,2); %Number of trials
    ...
    % f: N- Seed value defined for all runs
    mabdriver(a,b,0,0,0,f); % These two data sets should produce the same
    mabdriver(a,b,0,1,0,f); % sets of locations
    
    mabdriver(a,b,0,0,1,f); % These two data sets should produce the same
    mabdriver(a,b,0,1,1,f); % sets of locations

    mabdriver(a,b,1,0,0,f); % These two data sets should produce the same
    mabdriver(a,b,1,1,0,f); % sets of locations
    
    mabdriver(a,b,1,0,1,f); % These two data sets should produce the same
    mabdriver(a,b,1,1,1,f); % sets of locations    
end
